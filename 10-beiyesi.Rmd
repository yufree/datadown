...
# 看看AB对先验概率的影响
tidyr::crossing(x = seq(0.08, .35, .001), AB = c(1, 10, 100, 1000, 10000)) %>%
...
df <- tidyr::crossing(bats = c("L", "R"),
         AB = c(1, 10, 100, 1000, 10000))
df$mu <- predict(fit2, what = "mu", newdata = df)
df <- tidyr::crossing(df,
         x = seq(.1, .36, .0005))
df %>%
...
df %>%
  mutate(H = .3 * AB,
         mu = predict(fit2, what = "mu", newdata = df),
         sigma = predict(fit2, what = "sigma", newdata = df, type = "response"),
         alpha0 = mu / sigma,
         beta0 = (1 - mu) / sigma,
         alpha1 = alpha0 + H,
         beta1 = beta0 + AB - H,
...
plot_gamlss_fit <- function(f) {
  career3 %>%
    ...
    filter(bats != "B") %>%
    mutate(AB = 1000) %>%
    mutate(mu = predict(f, what = "mu", newdata = .),
           sigma = predict(f, what = "sigma", newdata = ., type = "response"),
           alpha0 = mu / sigma,
           beta0 = (1 - mu) / sigma,
...
players <- tidyr::crossing(year = c(1915, 1965, 2015),
                    bats = c("L", "R"),
                    H = 30,
                    AB = 100)
...
sim_replications <- career %>%
  tidyr::crossing(replication = 1:50) %>%
  mutate(p = rbeta(n(), alpha0, beta0),
         H = rbinom(n(), AB, p))
...
- [贝叶斯方法](http://mindhacks.cn/2008/09/21/the-magical-bayesian-method/)
- [贝叶斯镜像](https://github.com/mavelli/rocker-bayesian)
